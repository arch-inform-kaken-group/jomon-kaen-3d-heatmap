...
class MeaningMakingModel(nn.Module):
    def __init__(self,
        ...
        decoder_layer = nn.TransformerDecoderLayer(d_model=embed_dim, nhead=8, dim_feedforward=256,
        ...
        # 3-token auxiliary head conditioned on visual_context
    ...
    def _decode_and_predict(self, bottleneck, skip_features, target_tokens):
        ...
        visual_context = self.context_projection(visual_features)  # [B, hidden_dim]
        context_embed = self.context_to_embed(visual_context)
        # Compute 3-token auxiliary logits from visual_context
        aux_3tok_logits = None
        if self.training and target_tokens is not None:
            aux_3tok_logits = self.aux_3tok_head(visual_context)  # [B, 3 * V]
            aux_3tok_logits = aux_3tok_logits.view(-1, 3, self.vocab_size)  # [B, 3, V]
        if self.training and target_tokens is not None:
    ...
    def _forward_train(self, context_embed, target_tokens):
        ...
        input_tokens = torch.cat([torch.ones(B, 1, dtype=torch.long, device=device) * 1, target_tokens[:, :-1]], dim=1)
    ...
    def _forward_inference(self, context_embed):
        ...
        for t in range(1, max_len):
            ...
            tgt_mask = nn.Transformer.generate_square_subsequent_mask(T_curr, device=device)
            decoded = self.transformer_decoder(tgt, memory, tgt_mask=tgt_mask)
            logits = self.output_projection(decoded)
        ...
        if generated.size(1) < max_len:
            pad = torch.zeros(B, max_len - generated.size(1), dtype=torch.long, device=device)
...
class MeaningMakingLightningModule(pl.LightningModule):
    ...
    def configure_optimizers(self):
        params = [
            ...
            {'params': self.model.transformer_decoder.parameters(), 'lr': self.hparams.learning_rate},
            ...
            {'params': self.model.context_to_embed.parameters(), 'lr': self.hparams.learning_rate},
            ...
            {'params': self.model.aux_3tok_head.parameters(), 'lr': self.hparams.learning_rate * 0.8},  # NEW
