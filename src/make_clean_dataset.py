...
def refine_transcript_with_gemini(audio_file, raw_transcript_json):
        ...
        adaptation=cloud_speech.SpeechAdaptation( 
            phrase_sets=[ 
                cloud_speech.SpeechAdaptation.AdaptationPhraseSet( 
                    phrase_set=f'projects/{PROJECT_ID}/locations/global/phraseSets/malaysia' 
                ) 
            ] 
        )) 
        ...
        if data_paths.get('voice'): 
            ...
            if REFINE_TRANSCRIPT: 
                if Path(data_paths['transcript_save']).exists():
                    ...
                    refined_transcript = refine_transcript_with_gemini(
                        data_paths['voice_save'],
                        data_paths['transcript_save']
                    )
                    with open(data_paths['refined_transcript_save'], "w", encoding='utf-8') as f:
                        f.write(refined_transcript)
                else:
                    errors = increment_error('Raw transcript not found for refinement', 
