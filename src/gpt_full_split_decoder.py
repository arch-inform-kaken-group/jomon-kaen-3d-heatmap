...
class ChannelAttention(nn.Module):
    ...
    def __init__(self, bottleneck_dim):
        ...
        self.attn = nn.Sequential(
            ...
            nn.ReLU(), nn.Conv3d(bottleneck_dim // 4, 1, kernel_size=1),
            nn.Sigmoid())
...
class MeaningMakingModel(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.final_conv = nn.Conv3d(up_dims[-1],
                                    self.num_channels,
                                    kernel_size=1)
        ...
        self.aux_3tok_head = nn.Sequential(
            nn.Linear(hidden_dim, embed_dim), nn.ReLU(), nn.Dropout(0.3),
    ...
    def forward(self, x, target_tokens=None, return_bottleneck=False):
        ...
        emo_ctx = self.emotion_pooling(emotion_preds.detach()).view(
            batch_size, -1)  # [B, 5]
        heat_ctx = self.heatmap_pooling(heatmap_preds.detach()).view(
            batch_size, -1)  # [B, 1]
...
class MeaningMakingLightningModule(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.nonzero_reg_heat = NonZeroRegularization(
            weight=0.05, target_sparsity=self.hparams.nonzero_heat_target)
    ...
    def _shared_step(self, batch, is_training=True):
        ...
        emotion_preds, heatmap_preds, token_preds, aux_3tok_logits, flat_features = self.model(
            inputs, tokens if is_training else None)
        ...
        # Total loss
        voxel_loss = (self.hparams.emo_loss_weight * loss_emo +
                      self.hparams.heat_loss_weight * loss_heat +
                      self.hparams.global_loss_weight *
                      (loss_emo + loss_heat + loss_tok) + nonzero_emo +
                      nonzero_heat + self.hparams.l1_weight * 0.5 *
                      (l1_emo + l1_heat))
    ...
    def training_step(self, batch, batch_idx):
        loss, l_emo, l_heat, l_tok, l1_emo, l1_heat, _, _, nz_emo, nz_heat, aux_loss, aux_3tok_loss = self._shared_step(
    ...
    def validation_step(self, batch, batch_idx):
        loss, l_emo, l_heat, l_tok, l1_emo, l1_heat, emotion_preds, heatmap_preds, nz_emo, nz_heat, aux_loss, _ = self._shared_step(
        ...
        emo_iou = compute_iou(
            torch.sigmoid(emotion_preds) > 0.5, emotion_labels > 0.5)
        heat_iou = compute_iou(
            torch.sigmoid(heatmap_preds) > 0.5, heatmaps > 0.1)
        ...
        if batch_idx == 0 and self.global_rank == 0:
            ...
            if emo_mean < self.emo_sparsity:
                print(
                    f"[Rank {self.global_rank}] WARNING: Emotion pred mean = {emo_mean:.6f} < target {self.emo_sparsity:.6f}"
                )
            if heat_mean < self.heat_sparsity:
                print(
                    f"[Rank {self.global_rank}] WARNING: Heatmap pred mean = {heat_mean:.6f} < target {self.heat_sparsity:.6f}"
                )
    ...
    def configure_optimizers(self):
        params = [
            ...
            {
                'params': self.model.skip_blocks.parameters(),
                'lr': self.hparams.learning_rate
            },
            {
                'params': self.model.channel_attentions.parameters(),
                'lr': self.hparams.learning_rate * 1.5
            },
            {
                'params': self.model.final_conv.parameters(),
                'lr': self.hparams.learning_rate * 1.5
            },
            {
            ...
            {
                'params': self.model.gpt_decoder.parameters(),
                'lr': self.hparams.learning_rate
            },
            {
            ...
            {
                'params': self.model.context_to_prefix.parameters(),
                'lr': self.hparams.learning_rate
            },
            {
            ...
            {
                'params': self.model.aux_3tok_head.parameters(),
                'lr': self.hparams.learning_rate * 0.8
            },
        ]
        ...
        scheduler = torch.optim.lr_scheduler.SequentialLR(
            optimizer, [linear_warmup, cosine_scheduler],
...
if __name__ == "__main__":
    ...
    if len(all_data_paths) == 0:
        ...
        if num_gpus < 3:
            print(
                f"WARNING: Only {num_gpus} GPU(s) available. Using all available."
            )
        ...
        try:
            torchinfo.summary(model.cpu(),
                              input_size=(min(BATCH_SIZE,
                                              2), 3, VOXEL_RESOLUTION,
                                          VOXEL_RESOLUTION, VOXEL_RESOLUTION),
                              device="cpu")
        except Exception as e:
