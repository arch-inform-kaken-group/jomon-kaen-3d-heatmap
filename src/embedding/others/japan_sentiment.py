...
def tokenize_japanese(data_paths):
    ...
    for data_path in tqdm(data_paths, desc='TOKENIZING JAPANESE COMMENTS'):
        ...
        data_path['TOKENS'] = [
            m.normalized_form() for m in tokens if m.part_of_speech()[0] in parts_of_speech_to_keep
        ]
    return data_paths
...
def embed_tokens(data_paths, model, model_id, mode='fulltext'):
    ...
    embeddings = model.encode(
        ...
        normalize_embeddings=True, # Normalizing here is efficient for cosine similarity
...
if __name__ == "__main__":
    ...
    if not os.path.exists(root):
        ...
        # Step 1 & 2: Load data, tokenize, and calculate QA percentages
        ...
        embeddings = embed_tokens(data_paths, model, SELECTED_MODEL_ID, mode='fulltext')
        ...
        try:
            # 1. Identify indices for emotional labels and the neutral label.
            emotional_labels = ["面白い", "美しい", "不思議", "怖い"]
            neutral_label = "何も感じない"
            
            emotional_indices = [TARGET_LABELS_JP.index(L) for L in emotional_labels]
            neutral_idx = TARGET_LABELS_JP.index(neutral_label)
            
            # 2. Define the two poles of our new axis.
            # Emotional Pole: The average embedding of the four emotional labels.
            v_emotional_avg = np.mean(label_embeddings[emotional_indices], axis=0)
            # Neutral Pole: The embedding of the "Feel Nothing" label.
            v_neutral = label_embeddings[neutral_idx]
            
            # 3. Create the "emotionality" axis and normalize it.
            emotionality_axis = v_emotional_avg - v_neutral
            axis_norm = np.linalg.norm(emotionality_axis)
            if axis_norm > 0:
                emotionality_axis /= axis_norm
            
            # 4. Calculate the emotionality score for EACH comment.
            emotionality_scores = np.dot(embeddings, emotionality_axis)
            
            # 5. Adjust the original similarities using the emotionality score.
            adjusted_similarities = np.copy(similarities)
            weight = 3.0 # A tunable parameter.
            print(f"Applying emotionality adjustment with weight: {weight}")
            
            # Boost all four emotional labels for comments with a positive emotionality score.
            # This requires reshaping the scores to broadcast correctly across columns.
            adjusted_similarities[:, emotional_indices] += emotionality_scores[:, np.newaxis] * weight
            
            # Penalize the neutral label for comments with a positive emotionality score.
        ...
        # Step 5: Convert ADJUSTED similarities to percentages and calculate alignment
        print("\nCalculating embedding distributions and alignment scores from ADJUSTED data...")
        # IMPORTANT: Use the `adjusted_similarities` here.
        ...
        cluster_labels = np.argmax(adjusted_similarities, axis=1) # Use adjusted similarities
        ...
        title_text = f"類似度クエリに基づく3Dクラスタリング (感情度調整後)\nModel: {SELECTED_MODEL_ID}"
