...
import numba # Added for custom UMAP metric
...
# --- FONT CONFIGURATION ---
CUSTOM_FONT_PATH = 'C:/Windows/Fonts/simhei.ttf' 
...
try:
    ...
    print(f"Successfully configured Matplotlib to use font: {CUSTOM_FONT_PATH}")
except Exception as e:
    print(f"Warning: Could not set Matplotlib font. Plots may not render special characters. Error: {e}")
...
# --- CUSTOM UMAP DISTANCE FUNCTION ---
@numba.njit(fastmath=True)
def masked_cosine_distance(a, b):
    """
    Computes cosine distance, ignoring entries marked with -1.0 in either vector.
    """
    dot_product = 0.0
    norm_a = 0.0
    norm_b = 0.0
    ...
    for i in range(a.shape[0]):
        ...
        if is_valid_a and is_valid_b:
            ...
            norm_b += b[i] ** 2
            n_valid_indices += 1
    if n_valid_indices == 0 or norm_a == 0.0 or norm_b == 0.0:
        return 1.0
...
def load_data_paths(root=''):
    ...
    print(f"NUMBER OF VALID DATA ENTRIES: {len(data_paths)}")
    return data_paths
...
def embed_transcripts(data_paths, model, model_id):
    ...
    for data_path in tqdm(data_paths, desc='PREPARING TRANSCRIPTS FOR EMBEDDING'):
        with open(data_path['TRANSCRIPT'], 'r', encoding='utf-8') as f:
            content = f.read()
        if use_prefix: sentences.append("query: " + content)
        else: sentences.append(content)
    print(f"Encoding {len(sentences)} transcripts with model: {model_id}...")
    embeddings = model.encode(sentences, batch_size=128, convert_to_numpy=True, normalize_embeddings=False, show_progress_bar=True)
    return embeddings
...
def process_pottery_group(pottery_data):
    ...
    for i, session in enumerate(sessions):
        ...
        plot_buffer = create_comparison_plot(session['qa_percentages'], session['embedding_percentages'], title=f"Pottery {pottery_id} | Session {session['SESSION_ID']}")
...
def generate_alignment_report(data_paths, model_id, output_filename, max_workers=10, debug=False):
    ...
    try:
        ...
        PDF_FONT_TO_USE = CUSTOM_FONT_NAME
        print(f"Successfully registered font '{CUSTOM_FONT_PATH}' for PDF generation.")
    except Exception as e:
        print(f"Error: Could not register font '{CUSTOM_FONT_PATH}'. PDF text may not render correctly. Error: {e}")
        PDF_FONT_TO_USE = 'Helvetica'
    doc = SimpleDocTemplate(output_filename, pagesize=(8.5 * inch, 11 * inch))
    styles = getSampleStyleSheet()
    styles.add(ParagraphStyle(name='TitleStyle', fontName=PDF_FONT_TO_USE, fontSize=20, alignment=TA_CENTER, spaceAfter=20))
    styles.add(ParagraphStyle(name='HeaderStyle', fontName=PDF_FONT_TO_USE, fontSize=16, spaceAfter=12, spaceBefore=20))
    styles.add(ParagraphStyle(name='BodyStyle', fontName=PDF_FONT_TO_USE, fontSize=10, leading=14))
    styles.add(ParagraphStyle(name='TranscriptStyle', fontName=PDF_FONT_TO_USE, fontSize=8, leading=12, leftIndent=10, rightIndent=10))
...
if __name__ == "__main__":
    ...
    if not os.path.exists(root):
        print(f"Error: The data directory '{root}' does not exist.")
    elif not os.path.exists(CUSTOM_FONT_PATH):
        print(f"Error: The font file '{CUSTOM_FONT_PATH}' was not found.")
    else:
        # Steps 1-4: Load data, calculate QA%, and get transcript embeddings
        ...
        # Step 5: Calculate transcript emotion vectors and alignment scores
        ...
        for i, data_path in enumerate(data_paths):
            data_path['embedding_percentages'] = {label: embedding_percentages_all[i, j] for j, label in enumerate(TARGET_LABELS_EN)}
        ...
        # --- NEW Step 6: Prepare Emotion Vectors for Masked UMAP Clustering ---
        print("\n--- Preparing Emotion Vectors for Custom UMAP ---")
        confidence_threshold = 15.0 # % value. Scores below this will be ignored.
        print(f"Applying confidence threshold: scores < {confidence_threshold}% will be masked.")
        
        emotion_vectors_for_umap = embedding_percentages_all.copy()
        emotion_vectors_for_umap[emotion_vectors_for_umap < confidence_threshold] = -1.0

        # --- Step 7: Generate 3D Clustering Visualization using the Custom Metric ---
        print("\n--- Generating 3D Clustering Visualization with Custom Distance ---")
        # Color points by their original dominant emotion (before masking)
        cluster_labels = np.argmax(similarities, axis=1)

        print("Running UMAP with 'masked_cosine_distance' on emotion vectors...")
        umap_3d = umap.UMAP(
            metric=masked_cosine_distance,
            n_neighbors=15,
            min_dist=0.1,
            random_state=42,
            n_jobs=1
        ).fit_transform(emotion_vectors_for_umap) # Use the masked vectors
        ...
        title_text = f"3D Clustering of Transcript Emotion Vectors (Masked Cosine)\nModel: {SELECTED_MODEL_ID}"
        plt.title(title_text, fontsize=16)
        plt.tight_layout()
        model_name_for_file = SELECTED_MODEL_ID.replace('/', '_')
        plot_output_filename = f"cluster_plot_3d_masked_emotion_{model_name_for_file}.png"
        plt.savefig(plot_output_filename)
        print(f"\n3D Plot successfully saved to '{plot_output_filename}'")
        
        # --- Step 8: Generate the final PDF report (this uses the original, unmasked data) ---
