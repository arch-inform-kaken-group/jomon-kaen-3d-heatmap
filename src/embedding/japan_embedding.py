...
def process_pottery_group(pottery_data):
    ...
    for i, session in enumerate(sessions):
        ...
        table_style = TableStyle([('ALIGN', (0, 0), (-1, -1), 'CENTER'),
                                  ...
                                  ('RIGHTPADDING', (0,0), (-1,-1), 0)]) # Optional: remove padding
...
def generate_alignment_report(data_paths, model_id, output_filename, max_workers=10, debug=False):
    ...
    story.append(Paragraph(f"Analysis generated on: {datetime.now().strftime('%Y-%m-%d %H:%M')} in Petaling Jaya", styles['BodyStyle']))
...
if __name__ == "__main__":
    ...
    # SELECTED_MODEL_ID = 'google/embedding-gemma-2b'
