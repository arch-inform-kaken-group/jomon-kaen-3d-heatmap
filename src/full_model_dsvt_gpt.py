...
# --- DSVT Components (Encoder + Decoder) ---
...
class DSVTBackbone(nn.Module):
    ...
    def forward(self, x):
        ...
        return tokens, coords, batch_indices, (x_coord, y, z), (D, H, W), B
...
class DSVTDecoder(nn.Module):
    ...
    def forward(self, tokens, coords, batch_indices, xyz, dims, B):
        x_coord, y, z = xyz
        D, H, W = dims
        pos = torch.stack([x_coord, y, z], dim=1).float()
        ...
        logits_sparse = self.head(tokens)  # [N, 6]
...
class GPTCaptioner(nn.Module):
    ...
    def __init__(self, embed_dim, vocab_size, max_len, num_layers=2, nhead=8):
        super().__init__()
        self.embed_dim = embed_dim
    ...
    def forward(self, global_feat, target_tokens=None):
        ...
        if self.training and target_tokens is not None:
            ...
            input_emb = torch.cat([context, token_emb], dim=1)  # [B, T+1, E]
            ...
            logits = self.output_proj(out[:, :-1, :])  # predict next token
            return logits
        else:
            ...
            for t in range(self.max_len):
                ...
                if (next_token == 2).all():  # assuming 2 = <eos>
                    break
...
class DSVTFullModel(pl.LightningModule):
    ...
    def __init__(self, vocab_size, max_comment_len=32):
        super().__init__()
    ...
    def forward(self, x, tokens=None):
        ...
        tokens_enc, coords, batch_idx, xyz, dims, _ = enc_out
        dec_out = self.decoder(tokens_enc,
                               coords,
                               batch_idx,
                               xyz,
                               dims,
        ...
        # Global feature for captioning: mean of encoded tokens per batch
        max_batch = batch_idx.max().item() + 1
        global_feats = []
        for b in range(max_batch):
            mask_b = batch_idx == b
            if mask_b.any():
                feat_b = tokens_enc[mask_b].mean(dim=0)
            else:
                feat_b = torch.zeros_like(tokens_enc[0])
            global_feats.append(feat_b)
        global_feats = torch.stack(global_feats)  # [B, embed_dim]
    ...
    def on_validation_start(self):
        # Safely retrieve val_paths from datamodule (works in DDP/FSDP)
        if hasattr(self.trainer.datamodule, 'val_paths'):
            self.val_data_paths = self.trainer.datamodule.val_paths
                     ...
                     'val_dataset') and hasattr(
                         ...
                         'data_paths'):
            # fallback if val_paths not directly exposed
