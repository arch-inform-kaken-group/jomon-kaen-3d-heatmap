# Use attention pooling to allow for a deeper model
import os
    ...
    def forward(self, global_feat, target_tokens=None):
        ...
        if self.training and target_tokens is not None:
            ...
            for t in range(self.max_len):
                ...
                if (next_token == 2).all():  # assuming 2 is end token
                    break
...
class DSVTFullModel(pl.LightningModule):
    ...
    def forward(self, x, tokens=None):
        ...
        for b in range(B):
            ...
            if not mask_b.any():
                ...
                feat_b = attn_out.squeeze(1).squeeze(0)      # [64]
            global_feats.append(feat_b)
        global_feats = torch.stack(global_feats)  # [B, 64]
    ...
    def configure_optimizers(self):
        ...
        backbone_params = list(self.backbone.parameters()) + list(self.decoder.parameters()) + list(self.global_attn_pool.parameters()) + [self.pool_query]
