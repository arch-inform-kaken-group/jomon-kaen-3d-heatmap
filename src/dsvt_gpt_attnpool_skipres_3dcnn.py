...
class DSVTLayer(nn.Module):
    def __init__(self, embed_dim, axis='x', tau=36):
        ...
        # Define local 3D shape for reshaping τ tokens
        if tau == 125:
            self.local_shape = (5, 5, 5)
        elif tau == 36:
            self.local_shape = (3, 3, 4)  # D_l, H_l, W_l
        elif tau == 16:
        ...
        # Lightweight shared 3D CNN (depthwise + pointwise)
    ...
    def forward(self, tokens, coords, batch_idx, D, H, W):
        ...
        # Sort by axis within each batch
        if self.axis == 'x':
            sort_key = coords[:, 0]
        elif self.axis == 'y':
            sort_key = coords[:, 1]
        ...
        # Reshape into groups: [S, tau, C]
        tokens_sets = tokens_sorted.view(S, self.tau, self.embed_dim)

        # Apply 3D CNN on each group
        d_l, h_l, w_l = self.local_shape
        tokens_3d = tokens_sets.transpose(1, 2).reshape(S, self.embed_dim, d_l, h_l, w_l)  # [S, C, D_l, H_l, W_l]
        tokens_cnn = self.local_cnn(tokens_3d)
        tokens_cnn = tokens_cnn.reshape(S, self.embed_dim, self.tau).transpose(1, 2)  # [S, tau, C]

        # Residual: original + CNN output
...
class DSVTBackbone(nn.Module):
    def __init__(self, in_channels=3, embed_dim=64, resolution=80, tau=36, num_layers=6):
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros(1, resolution, resolution, resolution, embed_dim)
        )
...
class DSVTDecoder(nn.Module):
    ...
    def forward(self, tokens, coords, batch_indices, xyz, dims, B, early_features=None):
        ...
        logits_dense = torch.zeros(B * D * H * W, self.out_channels, dtype=logits_sparse.dtype, device=logits_sparse.device)
        logits_dense[coords] = logits_sparse
        return logits_dense.view(B, D, H, W, self.out_channels).permute(0, 4, 1, 2, 3)


# GPT Caption Decoder with Anti-Overfitting
class GPTCaptioner(nn.Module):
    def __init__(self, embed_dim, vocab_size, max_len, num_layers=3, nhead=4, dropout=0.4, layer_drop=0.1):
        super().__init__()
        self.embed_dim = embed_dim
    ...
    def forward(self, global_feat, target_tokens=None):
        ...
        context = self.context_proj(global_feat).unsqueeze(1)  # [B, 1, E]
        ...
        if self.training and target_tokens is not None:
            ...
            generated = torch.stack(generated, dim=1) if generated else torch.zeros(B, 0, device=device, dtype=torch.long)
            L = generated.size(1)
            if L < self.max_len:
                pad = torch.full((B, self.max_len - L), 0, dtype=torch.long, device=device)
                generated = torch.cat([generated, pad], dim=1)
            logits_out = torch.zeros(B, self.max_len, self.vocab_size, device=device)
...
class DSVTFullModel(pl.LightningModule):
    ...
    def forward(self, x, tokens=None):
        ...
        if enc_out[0] is None:
            ...
            cap_logits = torch.zeros(B, self.hparams.max_comment_len, self.hparams.vocab_size, device=x.device)
            return emo, gaze, cap_logits

        tokens_enc, coords, batch_idx, xyz, dims, _, early_features = enc_out
        dec_out = self.decoder(tokens_enc, coords, batch_idx, xyz, dims, B, early_features)
        emo = dec_out[:, :5]
        gaze = dec_out[:, 5:6]

        # Attention Pooling on enhanced tokens
        global_feats = []
        for b in range(B):
            mask_b = (batch_idx == b)
            if not mask_b.any():
                ...
                tokens_b = tokens_enc[mask_b].unsqueeze(0)
                query = self.pool_query.expand(1, -1, -1)
                attn_out, _ = self.global_attn_pool(query, tokens_b, tokens_b)
                feat_b = attn_out.squeeze(1).squeeze(0)
            global_feats.append(feat_b)
        global_feats = torch.stack(global_feats)

        cap_logits = self.captioner(global_feats, tokens if self.training else None)
        return emo, gaze, cap_logits
    ...
    def _shared_step(self, batch, stage):
        ...
        if cap_target.size(1) < cap_pred.size(1):
            pad = torch.zeros(cap_target.size(0), cap_pred.size(1) - cap_target.size(1), dtype=torch.long, device=cap_target.device)
            cap_target = torch.cat([cap_target, pad], dim=1)
        loss_cap = self.ce_loss(cap_pred.reshape(-1, self.hparams.vocab_size), cap_target.reshape(-1))
    ...
    def on_validation_start(self):
        ...
        elif hasattr(self.trainer.datamodule, 'val_dataset') and hasattr(self.trainer.datamodule.val_dataset, 'data_paths'):
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW([
            ...
            {'params': gpt_params, 'weight_decay': 0.1}
        ], lr=LEARNING_RATE)
        ...
        cosine_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=2, eta_min=1e-6)
