...
def _calculate_vertex_intensities(gaze_points_np, mesh, gaussian_denominator, spatial_error):
    ...
    for v_idx in tqdm(hit_indices, desc="Smoothing", leave=False):
        ...
        [_, indices, dists] = kdtree.search_radius_vector_3d(mesh_vertices_np[v_idx], spatial_error)
        if len(indices) > 1:
            gaussian_weights = np.exp(-np.asarray(dists)**2 / gaussian_denominator)
            for i, neighbor_idx in enumerate(indices):
                if neighbor_idx != v_idx:
                    interpolated_values[neighbor_idx] += hit_value * gaussian_weights[i]
...
def run_heatmap_comparison(args):
    ...
    all_pottery_ids = sorted(list(set(japan_data.keys()) | set(malaysia_data.keys())))
    ...
    for pottery_id in tqdm(all_pottery_ids, desc="Processing All Pottery"):
        ...
        for country, data in [('japan', japan_data), ('malaysia', malaysia_data)]:
            ...
            for inst in tqdm(instances, desc=f"{pottery_id} - {country}", leave=False):
                gaze_points = pd.read_csv(inst['pointcloud'], header=0).iloc[:, :3].to_numpy().astype(np.float64)
                mesh = o3d.io.read_triangle_mesh(inst['model'])
                if len(mesh.vertices) == num_vertices:
                    summed_intensities += _calculate_vertex_intensities(gaze_points, mesh, gaussian_denominator, args.hololens_error)

            all_intensities[country] = summed_intensities / len(instances) if instances else np.zeros(num_vertices)
        ...
        if jp_intensities is None or len(jp_intensities) != num_v: jp_intensities = np.zeros(num_v)
        if my_intensities is None or len(my_intensities) != num_v: my_intensities = np.zeros(num_v)

        # Normalize for visualization
        max_jp = np.max(jp_intensities); norm_jp = jp_intensities / max_jp if max_jp > 0 else jp_intensities
        max_my = np.max(my_intensities); norm_my = my_intensities / max_my if max_my > 0 else my_intensities

        jsd_score = helper.calculate_jensen_shannon_distance(jp_intensities, my_intensities)
        all_scores.append({'pottery_id': pottery_id, 'js_distance': jsd_score})

        cmap = plt.get_cmap(args.cmap)
        helper.save_colored_mesh(base_mesh, norm_jp, cmap, os.path.join(pottery_output_dir, "japan_average.ply"))
        helper.save_colored_mesh(base_mesh, norm_my, cmap, os.path.join(pottery_output_dir, "malaysia_average.ply"))
        ...
        o3d.io.write_triangle_mesh(os.path.join(pottery_output_dir, "difference.ply"), diff_mesh, write_ascii=True)
...
def run_qa_cluster(args):
    ...
    if combined_df.empty:
        print("No QA data found. Aborting."); return

    session_counts = pd.crosstab([combined_df['pottery_id'], combined_df['session_id']], combined_df['answer'])
    session_percentages = session_counts.div(session_counts.sum(axis=1), axis=0) * 100
    pottery_level_df = session_percentages.groupby('pottery_id').mean().fillna(0)

    # Clustering logic from original script
    print("Finding optimal K using Elbow Method...")
    inertia = [KMeans(n_clusters=k, random_state=42, n_init='auto').fit(pottery_level_df).inertia_
    ...
    plt.savefig("qa_cluster_elbow_plot.png"); plt.close()
    ...
    for k in range(2, args.max_k + 1):
        ...
        reduced_features = pca.fit_transform(pottery_level_df.iloc[:, :-1]) # Exclude cluster col

        plt.figure(figsize=(12, 8))
        cmap = plt.get_cmap('viridis', k)
        scatter = plt.scatter(reduced_features[:, 0], reduced_features[:, 1], c=cluster_labels, cmap=cmap)
        ...
        for i in range(k):
            ...
            helper.draw_ellipse(points, ax=plt.gca(), edgecolor=cmap(i / (k-1 if k>1 else 1)), facecolor='none', lw=2, linestyle='--')

        plt.title(f'K-Means Clustering (K={k}) of Pottery by QA Emotion')
        plt.legend(handles=scatter.legend_elements()[0], labels=[f'Cluster {i}' for i in range(k)])
        plt.savefig(os.path.join(output_dir, f'pca_cluster_plot_k{k}.png')); plt.close()

        for i in range(k):
            members = pottery_level_df[pottery_level_df[f'cluster_k{k}'] == i].index.tolist()
            if members:
                helper.create_cluster_collage(members, args.pottery_models_dir, i, output_dir)
...
def run_transcript_clustering(args):
    ...
    if not transcripts_dict:
        print("No transcripts found. Aborting."); return

    print(f"Loading zero-shot model: {args.model_id}")
    device = 0 if torch.cuda.is_available() and not args.force_cpu else -1
    classifier = pipeline("zero-shot-classification", model=args.model_id, device=device)
    ...
    for i, result in enumerate(all_results):
        ...
        records.append({'pottery_id': pottery_id, 'session_id': session_id, **score_dict})
    ...
    inertia = [KMeans(n_clusters=k, random_state=42, n_init='auto').fit(pottery_level_df).inertia_
               for k in range(1, min(11, len(pottery_level_df)))]
    plt.figure(); plt.plot(range(1, len(inertia) + 1), inertia, marker='o')
    plt.title('Elbow Method for Optimal K'); plt.xlabel('Clusters (K)'); plt.ylabel('Inertia')
    plt.savefig("transcript_cluster_elbow_plot.png"); plt.close()
    ...
    for k in range(2, args.max_k + 1):
        ...
        plt.figure(figsize=(12, 8))
        scatter = plt.scatter(reduced_features[:, 0], reduced_features[:, 1], c=cluster_labels, cmap=plt.get_cmap('viridis', k))
        for i, txt in enumerate(pottery_level_df.index):
            plt.annotate(txt, (reduced_features[i, 0], reduced_features[i, 1]))
        plt.title(f'K-Means Clustering (K={k}) of Pottery by Transcript Emotion')
        plt.legend(handles=scatter.legend_elements()[0], labels=[f'Cluster {i}' for i in range(k)])
        plt.savefig(os.path.join(output_dir, f'pca_cluster_plot_k{k}.png')); plt.close()
...
def run_qa_alignment(args):
    ...
    data_paths = helper.calculate_qa_emotion_percentages(data_paths, args.language)
    ...
    for item in tqdm(data_paths, desc="Classifying Transcripts"):
        ...
        score_dict = {label: score for label, score in zip(result['labels'], result['scores'])}
        item['embedding_percentages'] = {label: score_dict.get(label, 0) * 100 for label in target_labels}
        ...
        if norm_qa > 0 and norm_embed > 0:
            item['alignment_score'] = np.dot(qa_vec, embed_vec) / (norm_qa * norm_embed)
        else:
            item['alignment_score'] = 0.0

    output_filename = f"{args.language.upper()}_QA_Transcript_Alignment_Report.pdf"
    helper.generate_alignment_report(data_paths, args.model_id, output_filename, args.language, args.font_path)
...
def run_word_frequency_analysis(args):
    ...
    if args.language == 'japan':
        ...
        for text in transcripts_dict.values():
            ...
            tokens = [m.normalized_form() for m in tokenizer.tokenize(normalized, mode) if m.part_of_speech()[0] in pos_to_keep]
    ...
    helper.generate_word_cloud_and_bar_chart(all_words, args.font_path, output_prefix)
    helper.generate_transcript_pdf(transcripts_by_pottery, f"{args.language}_transcripts.pdf", args.font_path)
...
def run_voxel_analysis(args):
    ...
    if not os.path.isdir(args.voxel_dir):
        print(f"Error: Directory not found: {args.voxel_dir}"); return

    ply_files = [f for f in os.listdir(args.voxel_dir) if f.endswith('.ply')]
    if not ply_files:
        print("No .ply files found."); return

    counts = {os.path.splitext(f)[0]: helper.read_ply_vertex_count(os.path.join(args.voxel_dir, f)) for f in ply_files}
    sorted_items = sorted(counts.items(), key=lambda item: item[1])
    ids, values = zip(*sorted_items)

    plt.figure(figsize=(15, 8))
    plt.bar(ids, values)
    plt.title('Voxel Count per Pottery ID'); plt.ylabel('Number of Voxels')
    plt.xticks(rotation=90); plt.tight_layout()
    plt.savefig("voxel_counts_barchart.png"); plt.close()
...
def run_label_embedding_visualization(args):
    ...
    embedding_2d = umap.UMAP(n_components=2, random_state=42, min_dist=0.0, metric="cosine", n_jobs=1).fit_transform(embeddings)
    embedding_3d = umap.UMAP(n_components=3, random_state=42, min_dist=0.0, metric="cosine", n_jobs=1).fit_transform(embeddings)

    colors = [plt.cm.get_cmap('jet', len(jp_labels))(i) for i in range(len(jp_labels))] * 2

    fig = plt.figure(figsize=(18, 8))
    ax_2d = fig.add_subplot(1, 2, 1)
    ax_2d.scatter(embedding_2d[:, 0], embedding_2d[:, 1], c=colors)
    for i, label in enumerate(all_labels): ax_2d.text(embedding_2d[i, 0], embedding_2d[i, 1], label)
    ax_2d.set_title('2D UMAP Projection of Emotion Labels')
    ax_3d = fig.add_subplot(1, 2, 2, projection='3d')
    ax_3d.scatter(embedding_3d[:, 0], embedding_3d[:, 1], embedding_3d[:, 2], c=colors)
    for i, label in enumerate(all_labels): ax_3d.text(embedding_3d[i, 0], embedding_3d[i, 1], embedding_3d[i, 2], label)
    ax_3d.set_title('3D UMAP Projection of Emotion Labels')

    plt.tight_layout(); plt.savefig("label_embedding_visualization.png"); plt.show()

# MAIN EXECUTION BLOCK
def main():
    parser = argparse.ArgumentParser(description="Run various analyses on Jomon Kaen dataset.")
    subparsers = parser.add_subparsers(dest="command", required=True, help="Available analyses")

    # 1. Heatmap Parser
    p1 = subparsers.add_parser('heatmap', help="Compare gaze heatmaps between Japan and Malaysia.")
    p1.add_argument('--japan_dir', type=str, default="./src/jomon_kaen_dataset/japan")
    p1.add_argument('--malaysia_dir', type=str, default="./src/jomon_kaen_dataset/malaysia")
    p1.add_argument('--output_dir', type=str, default="./processed/heatmap_comparison")
    ...
    # 2. QA Cluster Parser
    p2 = subparsers.add_parser('qa_cluster', help="Cluster pottery based on QA event emotion data.")
    ...
    p3 = subparsers.add_parser('transcript_cluster', help="Cluster pottery based on transcript emotion analysis.")
    ...
    p4 = subparsers.add_parser('qa_alignment', help="Generate a PDF report comparing QA and transcript emotions.")
    ...
    p5 = subparsers.add_parser('word_freq', help="Analyze word frequencies in transcripts.")
    ...
    p6.add_argument('--voxel_dir', type=str, default="./src/jomon_kaen_dataset/processed/voxel_pottery")
    p6.set_defaults(func=run_voxel_analysis)

    # 7. Label Embedding Parser
    p7 = subparsers.add_parser('label_viz', help="Visualize semantic embeddings of emotion labels.")
    p7.add_argument('--model_id', type=str, default='sentence-transformers/paraphrase-multilingual-mpnet-base-v2')
