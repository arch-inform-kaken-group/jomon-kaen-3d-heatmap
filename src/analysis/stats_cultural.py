...
def hp_p_from_f(f, d1, d2):
    if f is None or not np.isfinite(f): return np.nan
    if f <= 0: return "1.000000000000000e+00"
    if MP_OK:
        mpmath.mp.dps = 50
        x = mpmath.mpf(d2) / (mpmath.mpf(d2) + mpmath.mpf(d1) * mpmath.mpf(f))
        return _fmt_mp(mpmath.betainc(mpmath.mpf(d2) / 2, mpmath.mpf(d1) / 2, 0, x, regularized=True))
...
def p_is_significant(p):
    ...
    if isinstance(p, float) and np.isnan(p): return False
    s = str(p)
    if s.startswith('<'): return True
    try: return float(s) < 0.05
    except ValueError: return False

# ==========================================
# 1. DATA LOADING (UPDATED WITH COUNTRY)
# ==========================================
def load_multilingual_dataset(root_dirs):
    qa_records = []
    for root_dir in root_dirs:
        ...
        # Infer Country/Region from directory path
                ...
                for pottery_path in session_path.iterdir():
                    ...
                    if qa_file.exists():
                        try:
                            ...
                            df_temp['Language'] = language
                            df_temp['Subject'] = subject_id
                            df_temp['Country'] = country  # <-- NEW: Add Country
                            qa_records.append(df_temp)
                        except Exception as e:
                            print(f"Error loading {qa_file}: {e}")
...
# ==========================================
# 2. CALCULATE NORMALIZED METRICS (UPDATED WITH COUNTRY)
# ==========================================
def calculate_metrics_long(df_qa):
    print("Calculating session duration blocks and normalized metrics (Long Format)...")
    df = df_qa.copy()
    ...
    # Include 'Country' in session aggregations
    session_events = blocks.groupby(['pottery_id', 'session_id', 'Subject', 'Language', 'Country', 'emotion'])['event_count'].sum().unstack(fill_value=0).reset_index()
    session_duration = blocks.groupby(['pottery_id', 'session_id', 'Subject', 'Language', 'Country', 'emotion'])['duration'].sum().unstack(fill_value=0).reset_index()
...
def attach_features(pottery_df, features_file):
    ...
    try:
        if features_file.endswith('.xlsx'):
            ...
            try: feat_df = pd.read_csv(features_file, encoding='utf-8-sig')
            except: feat_df = pd.read_csv(features_file, encoding='shift_jis')
    except Exception as e:
        print(f"Error loading features file: {e}")
        return pottery_df.copy(), [], []
...
def run_anova_on_groups(df, group_col, metric_cols):
    ...
    for emo in metric_cols:
        if is_long_format:
            emo_df = df[df['Emotion'] == emo]
            group_data = {name: group['Response'].dropna().values for name, group in emo_df.groupby(group_col) if len(group['Response'].dropna()) > 0}
        else:
            group_data = {name: group[emo].dropna().values for name, group in df.groupby(group_col) if len(group[emo].dropna()) > 0}
        ...
        if len(valid_groups) >= 2:
            ...
            if d2 >= 1:
                ...
                try:
                    ...
                    f_stat, f_p = np.nan, np.nan
                results.append({
                    'Emotion': emo, 'Total_N': total_n, 'Group_Details': " | ".join(group_details),
                    'ANOVA_F_Stat': f_stat, 'ANOVA_df1': d1, 'ANOVA_df2': d2,
                    'ANOVA_p_value': f_p, 'ANOVA_Significant': p_is_significant(f_p)
                })
    return pd.DataFrame(results)
...
def run_lmm_on_long(long_df, fixed_effect_col, is_categorical=True):
    ...
    for emo in EMOTION_COLS:
        ...
        results.append({
            'Emotion': emo, 'Total_Observations': total_n, 'Group_Details': " | ".join(group_details),
            'LMM_F_Stat': f_stat, 'LMM_p_value': p_val, 'LMM_Significant': p_is_significant(p_val),
            'Subject_Variance': subj_var, 'Residual_Variance': resid_var,
            'ICC': subj_var / (subj_var + resid_var) if pd.notna(subj_var) and pd.notna(resid_var) and (subj_var + resid_var) > 0 else np.nan,
            'Model_Used': 'LMM' if lmm_success else 'OLS_ANOVA_Fallback'
        })
    return pd.DataFrame(results)
...
# ==========================================
# 6. NEW: REGION COMPARISON HELPER
# ==========================================
def analyze_region_subset(df_events, df_dur, mask, subset_name):
    ...
    df_e_sub = df_events[mask].copy()
    df_d_sub = df_dur[mask].copy()
    
    # Ensure we have both countries in the subset
...
def main():
    print("=" * 80)
    print("STATISTICAL ANALYSIS: CLASSICAL ANOVA + LMM + REGION COMPARISONS")
    print("=" * 80)
    ...
    print("Running 2-2: Typology Analysis...")
    a22_e_anova_list, a22_d_anova_list, a22_e_lmm_list, a22_d_lmm_list = [], [], [], []
    for shape in shape_cols:
        res_e_a = run_anova_on_groups(pot_events_feat, shape, EMOTION_COLS)
        res_d_a = run_anova_on_groups(pot_dur_feat, shape, EMOTION_COLS)
        if not res_e_a.empty: res_e_a.insert(0, 'Typology', shape.replace('SHAPE_TYPE_', '')); a22_e_anova_list.append(res_e_a)
    ...
    print("Running 2-4: Feature Analysis...")
    a24_e_anova_list, a24_d_anova_list, a24_e_lmm_list, a24_d_lmm_list = [], [], [], []
    for feat in feature_cols:
        res_e_a = run_anova_on_groups(pot_events_feat, feat, EMOTION_COLS)
        res_d_a = run_anova_on_groups(pot_dur_feat, feat, EMOTION_COLS)
        if not res_e_a.empty: res_e_a.insert(0, 'Feature', feat); a24_e_anova_list.append(res_e_a)
    ...
    # ==========================================
    # NEW: 3-x REGION COMPARISONS (JAPAN vs MALAYSIA)
    # ==========================================
    ...
    r_pot_e_a, r_pot_d_a, r_pot_e_l, r_pot_d_l = analyze_region_subset(
        sess_events_feat_long, sess_dur_feat_long, sess_events_feat_long['Type'] == 'Pottery', 'Pottery')
        
    # 3-2 Dogu vs Dogu
    r_dog_e_a, r_dog_d_a, r_dog_e_l, r_dog_d_l = analyze_region_subset(
        sess_events_feat_long, sess_dur_feat_long, sess_events_feat_long['Type'] == 'Dogu', 'Dogu')
        
    # 3-3 Typology vs Typology
    r_typ_e_a, r_typ_d_a, r_typ_e_l, r_typ_d_l = [], [], [], []
    for shape in shape_cols:
        mask = sess_events_feat_long[shape] == 1
        a_e, a_d, l_e, l_d = analyze_region_subset(sess_events_feat_long, sess_dur_feat_long, mask, shape)
        if not a_e.empty: a_e.insert(0, 'Typology', shape.replace('SHAPE_TYPE_', '')); r_typ_e_a.append(a_e)
        if not a_d.empty: a_d.insert(0, 'Typology', shape.replace('SHAPE_TYPE_', '')); r_typ_d_a.append(a_d)
        if not l_e.empty: l_e.insert(0, 'Typology', shape.replace('SHAPE_TYPE_', '')); r_typ_e_l.append(l_e)
        if not l_d.empty: l_d.insert(0, 'Typology', shape.replace('SHAPE_TYPE_', '')); r_typ_d_l.append(l_d)
    ...
    for feat in feature_cols:
        ...
        a_e, a_d, l_e, l_d = analyze_region_subset(sess_events_feat_long, sess_dur_feat_long, mask, feat)
        if not a_e.empty: a_e.insert(0, 'Feature', feat); r_feat_e_a.append(a_e)
        if not a_d.empty: a_d.insert(0, 'Feature', feat); r_feat_d_a.append(a_d)
        if not l_e.empty: l_e.insert(0, 'Feature', feat); r_feat_e_l.append(l_e)
        if not l_d.empty: l_d.insert(0, 'Feature', feat); r_feat_d_l.append(l_d)
    ...
    with pd.ExcelWriter(out_file, engine='openpyxl') as writer:
        ...
        # 3-x NEW REGION COMPARISON SHEETS
        if not r_pot_e_a.empty: r_pot_e_a.to_excel(writer, sheet_name='3-1_Region_Pottery_ANOVA_E', index=False)
        if not r_pot_d_a.empty: r_pot_d_a.to_excel(writer, sheet_name='3-1_Region_Pottery_ANOVA_D', index=False)
        if not r_pot_e_l.empty: r_pot_e_l.to_excel(writer, sheet_name='3-1_Region_Pottery_LMM_E', index=False)
        if not r_pot_d_l.empty: r_pot_d_l.to_excel(writer, sheet_name='3-1_Region_Pottery_LMM_D', index=False)
        
        if not r_dog_e_a.empty: r_dog_e_a.to_excel(writer, sheet_name='3-2_Region_Dogu_ANOVA_E', index=False)
        if not r_dog_d_a.empty: r_dog_d_a.to_excel(writer, sheet_name='3-2_Region_Dogu_ANOVA_D', index=False)
        if not r_dog_e_l.empty: r_dog_e_l.to_excel(writer, sheet_name='3-2_Region_Dogu_LMM_E', index=False)
        if not r_dog_d_l.empty: r_dog_d_l.to_excel(writer, sheet_name='3-2_Region_Dogu_LMM_D', index=False)
        
        if not r_typ_e_a.empty: r_typ_e_a.to_excel(writer, sheet_name='3-3_Region_Typology_ANOVA_E', index=False)
        if not r_typ_d_a.empty: r_typ_d_a.to_excel(writer, sheet_name='3-3_Region_Typology_ANOVA_D', index=False)
        if not r_typ_e_l.empty: r_typ_e_l.to_excel(writer, sheet_name='3-3_Region_Typology_LMM_E', index=False)
        if not r_typ_d_l.empty: r_typ_d_l.to_excel(writer, sheet_name='3-3_Region_Typology_LMM_D', index=False)
        
        if not r_feat_e_a.empty: r_feat_e_a.to_excel(writer, sheet_name='3-4_Region_Features_ANOVA_E', index=False)
        if not r_feat_d_a.empty: r_feat_d_a.to_excel(writer, sheet_name='3-4_Region_Features_ANOVA_D', index=False)
        if not r_feat_e_l.empty: r_feat_e_l.to_excel(writer, sheet_name='3-4_Region_Features_LMM_E', index=False)
        if not r_feat_d_l.empty: r_feat_d_l.to_excel(writer, sheet_name='3-4_Region_Features_LMM_D', index=False)
